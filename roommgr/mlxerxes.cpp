...
void Xerxes::init ( void )
{
    ...
    emote->add ( "laughs at the 'gods'." );
    emote->add ( "is unimpressed by you." );
    emote->add ( "spits on your boots." );
    emote->add ( "says 'You are nothing to me.'" );
	emote->add ( "says 'Duach? Nothing but a pissant.'" );
	emote->add ( "points and laughs at your weapon." );
	emote->add ( "mocks the size of your spellbook." );
	emote->add ( "doesn't think a peasant such as yourself is worth his time." );
	emote->add ( "laughs at you, 'You think you can withstand my power?'" );
}

int Xerxes::normalLogic ( void )
{
	int retVal = 10;
...
int Xerxes::spellAttack ( void ) 
{
	// xerxes should cast elemental + necromancer spells -
	...
	if ( random ( 0, 1 ) )
	{
		...
		if ( necroSkill )
		{
			switch ( random ( 0, 6 ) )
			{
				case 0:
				// summon dragon
				setAction ( new CombatCastSpell ( character, _SPELL_65, combatTarget->servID, 0, 0 ) );
				return 1;
