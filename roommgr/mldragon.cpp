...
void Dragon::init ( void )
{
	...
	emote->add ( "blows smoke from its' nose." );
	emote->add ( "slams its heavy tail on the ground." );
    emote->add ( "flexes its' wings." );
    emote->add ( "picks at some scales with a fang." );
    emote->add ( "roars fiercly." );
    emote->add ( "emits a sinister growl." );
	emote->add ( "coughs up a fireball." );
	emote->add ( "takes flight for a moment, then slams back down." );
}

int Dragon::normalLogic ( void )
{
	int retVal = 10;
...
int Dragon::spellAttack ( void ) 
{
	// Dragon should cast elemental + necromancer spells -
