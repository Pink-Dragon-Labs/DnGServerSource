	...
	switch ( random ( 0, 6 ) ) {
		...
		case 3: {
			...
			while ( element ) {
				...
				if ( !chance && !thePlayer->character->hasAffect ( _AFF_JAIL ) && zone->allowCombat() && !thePlayer->isNPC && !thePlayer->character->hidden && !thePlayer->character->combatGroup && !isGroupMember ( thePlayer ) && thePlayer->character->health > 0 && !thePlayer->checkAccess ( _ACCESS_MODERATOR ) )  {
					if ( character->level > 1 ) {
						...
						emote ( "eyes %s hungrily.", thePlayer->getName() );
					}
